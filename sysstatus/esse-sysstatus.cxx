	...
	try {
		...
		if (state.cpu || state.gpu || state.printers || state.power || state.windows) {
			...
			if (state.power) {
				...
				else if (desc.status == Power::PowerStatus::BatteryCharging) ps = FormatString(Localized(403), uint(desc.battery_charge_level * 100.0));
				else if (desc.status == Power::PowerStatus::BatteryDischarging) ps = FormatString(Localized(404), uint(desc.battery_charge_level * 100.0));
