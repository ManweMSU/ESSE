		...
		namespace Format
		{
			...
			InterfaceTemplateImage::InterfaceTemplateImage(Streaming::Stream * Source, const string & Locale, const string & System, double Scale) : InterfaceTemplateImage()
			{
				try {
					...
					{
						ArchiveFile manifest_file = container->FindArchiveFile(U"IMF", 1);
					...
					{
						for (int i = 1; i <= container->GetFileCount(); i++) if (container->GetFileType(i) == U"STT") {
							string lang = container->GetFileName(i);
							if (!lang.Length() || !Locale.Length() || lang == Locale) {
								if (!lang.Length()) lang = U"_";
					...
					for (int i = 0; i < manifest.Assets.Length(); i++) {
						if (!manifest.Assets[i].SystemFilter.Length() || !System.Length() || manifest.Assets[i].SystemFilter == System) {
							...
							ArchiveFile asset_file = container->FindArchiveFile(U"AST", asset_id);
							if (asset_file) {
					...
					for (int a = 0; a < Assets.Length(); a++) {
						for (int i = 0; i < Assets[a].Textures.Length(); i++) {
							...
							ArchiveFile texture_file = container->FindArchiveFile(U"IMG", texture_ref.ImageID);
					...
					{
						ArchiveFile str_map_file = container->FindArchiveFile(U"STM", 1);
						if (str_map_file) {
			...
			void InterfaceTemplateImage::Encode(Streaming::Stream * Output, uint32 Flags)
			{
				...
				for (int a = 0; a < Assets.Length(); a++) {
					...
					container->SetFileType(current, U"AST");
				...
				for (int i = 0; i < locale_count; i++) {
					...
					container->SetFileType(current, U"STT");
					container->SetFileID(current, i + 1);
					container->SetFileName(current, (locale->GetValue().key == U"_") ? U"" : locale->GetValue().key);
				...
				for (int i = 0; i < texture_count; i++) {
					...
					Codec::EncodeImage(&texture_stream, texture->GetValue().value, U"EIWV");
					texture_stream.Seek(0, Begin);
					container->SetFileType(current, U"IMG");
				...
				if (Flags & EncodeFlags::EncodeStringNames) {
					...
					container->SetFileType(current, U"STM");
			...
			Template::Shape * CompileShape(InterfaceTemplateImage * image, InterfaceShape & shape, InterfaceTemplate & Template, IResourceResolver * ResourceResolver)
			{
				if (shape.Class == U"Frame") {
					...
					for (int i = 0; i < shape.IntegerValues.Length(); i++) {
						if (shape.IntegerValues[i].Name == U"RenderMode") {
					...
					int gp = 0;
					for (int i = 0; i < shape.DoubleValues.Length(); i++) {
						if (shape.DoubleValues[i].Name == U"Gradient") {
							result->Gradient[gp].Position = shape.DoubleValues[i].ToTemplate();
							gp++;
						} else if (shape.DoubleValues[i].Name == U"GradientAngle") {
					...
					for (int i = 0; i < shape.RectangleValues.Length(); i++) {
						if (shape.RectangleValues[i].Name == U"Position") {
							result->Position = shape.RectangleValues[i].ToTemplate();
						} else if (shape.RectangleValues[i].Name == U"GradientPoints") {
					...
					result->Retain();
					return result;
				} else if (shape.Class == U"Inversion") {
					...
					for (int i = 0; i < shape.StringValues.Length(); i++) {
						if (shape.StringValues[i].Name == U"Texture") {
					...
					for (int i = 0; i < shape.IntegerValues.Length(); i++) {
						...
						} else if (shape.IntegerValues[i].Name == U"VerticalAlign") {
			...
			Template::ControlTemplate * CompileControl(InterfaceTemplateImage * image, InterfaceControl & control, InterfaceTemplate & Template, InterfaceTemplate * Style, IResourceResolver * ResourceResolver, Array<DialogReference> & References)
			{
				...
				if (base) {
					...
					if (Style) base_control = Style->Dialog[U"@default:" + control.Class];
					if (base_control) {
			...
			void InterfaceTemplateImage::Specialize(const string & Locale, const string & System, double Scale)
			{
				...
				if (Locale.Length()) {
					...
					while (locale) {
						...
						if (locale->GetValue().key != U"_" && locale->GetValue().key != Locale) Locales.BinaryTree::Remove(locale);
						locale = next;
					}
				}
			...
			string InterfaceTemplateImage::GetStringByID(int ID)
			{
				try {
					for (auto & locale : Locales) {
						if (locale.key == U"_") continue;
						return locale.value->GetString(ID);
					}
				} catch (...) {}
				try {
					auto global = Locales[U"_"];
			...
			string InterfaceTemplateImage::GetStringByName(const string & name)
			{
				...
				if (p_id) return GetStringByID(*p_id); else return U"";
			}
