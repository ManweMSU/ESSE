...
namespace Engine
{
	namespace UI
	{
		namespace Controls
		{
			...
			{
				if (Template->Properties->GetTemplateClass() != U"Edit") throw InvalidArgumentException();
			...
			string Edit::GetControlClass(void) { return U"Edit"; }
			void Edit::Undo(void)
			{
			...
			{
				if (Template->Properties->GetTemplateClass() != U"MultiLineEdit") throw InvalidArgumentException();
			...
			void MultiLineEdit::SetText(const string & text)
			{
				...
				while (sp < chars.Length()) {
					int ep = sp;
					while (ep < chars.Length() && chars[ep] != U'\n') ep++;
					nle = chars[ep] == U'\n';
					int vcc = 0;
					for (int i = sp; i < ep; i++) if (chars[i] >= 0x20 || chars[i] == U'\t') vcc++;
					_content.lines << EditorLine();
					_content.lines.LastElement().text.SetLength(vcc);
					int wp = 0;
					for (int i = sp; i < ep; i++) if (chars[i] >= 0x20 || chars[i] == U'\t') { _content.lines.LastElement().text[wp] = chars[i]; wp++; }
					sp = ep + 1;
				}
			...
			string MultiLineEdit::GetControlClass(void) { return U"MultiLineEdit"; }
			...
			string MultiLineEdit::FilterInput(const string & input)
			{
				...
				for (int i = 0; i < utf32.Length(); i++) {
					bool enabled = false;
					if (utf32[i] >= 0x20 || utf32[i] == U'\t' || utf32[i] == U'\n') {
						if (_chars_enabled.Length()) for (int j = 0; j < _chars_enabled.Length(); j++) {
							if (utf32[i] == _chars_enabled[j]) { enabled = true; break; }
						} else enabled = true;
					}
