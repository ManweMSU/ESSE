...
namespace Engine
{
	namespace UI
	{
		namespace Controls
		{
			...
			{
				if (Template->Properties->GetTemplateClass() != U"MenuBar") throw InvalidArgumentException();
				...
				for (auto & c : Template->Children) {
					if (c.Properties->GetTemplateClass() != U"MenuBarElement") throw InvalidArgumentException();
			...
			string MenuBar::GetControlClass(void) { return U"MenuBar"; }
