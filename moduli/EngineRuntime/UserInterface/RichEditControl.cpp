		...
		namespace Controls
		{
			namespace RichEditContents
			{
				...
				public:
					...
					virtual void SerializeToPlainText(const Array<int> & index, Array<uint32> & text, uint64 attr_lo, uint64 attr_hi) override
					{
						if (_image) {
							...
							Streaming::MemoryStream stream(0x10000);
							Codec::EncodeImage(&stream, _image, U"EIWV");
				...
				class TextContents : public RichEdit::ContentBox
				{
					...
					static bool _is_ident_char(const TaggedChar & chr) { return !(chr.lo & FlagLargeObject) && (IsAlphabetical(chr.lo & MaskUcsCode) || ((chr.lo & MaskUcsCode) >= U'0' && (chr.lo & MaskUcsCode) <= U'9') || ((chr.lo & MaskUcsCode) == U'_')); }
				public:
						...
						if (!_words.Length()) {
							...
							while (pos < _text.Length()) {
								...
								if ((_text[pos].lo & MaskUcsCode) == U'\n' && !(_text[pos].lo & FlagLargeObject)) {
									...
									pos++;
								} else if ((_text[pos].lo & MaskUcsCode) == U'\t' && !(_text[pos].lo & FlagLargeObject)) {
									_words << Word();
									...
									while (pos < _text.Length() && !(_text[pos].lo & FlagLargeObject) &&
										...
										(_text[pos].lo & MaskUcsCode) != U'\n' && (_text[pos].lo & MaskUcsCode) != U'\t') pos++;
									_words << Word();
					...
					virtual void ClearSelection(int * pos1, ContentBox ** box1, int * pos2, ContentBox ** box2) override
					{
						...
						while (pos < _text.Length()) {
							...
							int sp = pos;
							while (pos < _text.Length() && (_text[pos].lo & FlagLargeObject || (_text[pos].lo & MaskUcsCode) != U'\n')) pos++;
						...
						while (pos < text.Length()) {
							if (text[pos] == 27) {
								...
								if (text[pos] == U'e') { pos++; break; }
									...
									uint64 clr = (clr_code == U"********") ? uint32(_edit->DefaultTextColor) : clr_code.ToUInt32(HexadecimalBase);
									pos += 8;
									...
									uint64 height = (height_code == U"****") ? _edit->DefaultFontHeight : height_code.ToUInt32(HexadecimalBase);
									pos += 4;
									DeserializeFromPlainTextRecurrent(index, text, pos, at, (attr_lo & ~MaskFontHeight) | (height << 48), attr_hi);
								} else if (text[pos] == U'a') {
									pos++;
									uint32 dg[2];
									dg[0] = text[pos];
									dg[1] = 0;
							...
							} else if (text[pos] >= 32 || text[pos] == U'\n' || text[pos] == U'\t') {
							...
							if ((_text[pos].lo & 0xFFFFFFEF00000000) == attr_lo && (_text[pos].hi & 0xFFFFFFFF) == attr_hi) {
								...
								if ((_text[pos].lo & 0xFFFFFFEF00000000) == prev_attr_lo && (_text[pos].hi & 0xFFFFFFFF) == prev_attr_hi) {
									...
									if (((new_attr_lo | attr_lo) & MaskStyleAttributes) != (new_attr_lo & MaskStyleAttributes)) {
										...
										text << U'\33'; text << U'f'; text << code[0]; text << code[1];
										...
										text << U'\33'; text << U'h'; text << code[0]; text << code[1]; text << code[2]; text << code[3];
										...
										text << U'\33'; text << U'a'; text << (U'1' + fa);
					...
					virtual void SerializeRange(const Array<int> & index, Array<uint32> & text, int from, int to, uint64 attr_lo, uint64 attr_hi) override
					{
						if (attr_hi == 0xFFFFFFFFFFFFFFFF) {
							...
							for (int i = 0; i < index.Length(); i++) if (index[i] >= 0) {
								...
								text << U'\33'; text << U'n'; text << ucs; text << U'\33'; text << U'e';
							}
							int attr_src = -1;
					...
					virtual void SetAttributesForRange(int from, int to, uint64 attr_lo, uint64 attr_hi, uint64 attr_lo_mask, uint64 attr_hi_mask) override
					{
						...
						while (pos < to) {
							...
							int sp = pos;
							int ep = pos;
							while (sp >= 0 && (_text[sp].lo & FlagLargeObject || (_text[sp].lo & MaskUcsCode) != U'\n')) sp--;
							sp++;
							while (ep < _text.Length() && (_text[ep].lo & FlagLargeObject || (_text[ep].lo & MaskUcsCode) != U'\n')) ep++;
							if (ep < _text.Length() && (_text[ep].lo & MaskUcsCode) == U'\n') ep++;
			...
			{
				if (Template->Properties->GetTemplateClass() != U"RichEdit") throw InvalidArgumentException();
			...
			{
				if (text[0] == U'\33' && text[1] == U'X') SetAttributedText(text.Fragment(2, -1).NormalizedForm());
				else SetAttributedText(U"\33n*\33e\33f00\33h****\33c********" + text + U"\33e\33e\33e");
				Invalidate();
			}
			...
			string RichEdit::GetControlClass(void) { return U"RichEdit"; }
			...
			void RichEdit::InsertImage(Codec::Image * image)
			{
				...
				Streaming::MemoryStream stream(0x10000);
				Codec::EncodeImage(&stream, image, U"EIWV");
				string code = U"\33p" + string(width, HexadecimalBase, 4) + string(height, HexadecimalBase, 4) +
					ConvertToBase64(stream.GetBuffer(), int(stream.Length())) + U"\33e";
				_undo.PushCurrentVersion();
				_save = true;
				if (!IsSelectionEmpty() && _caret_box) _caret_box->ClearSelection(&_caret_pos, &_caret_box, &_selection_pos, &_selection_box);
				if (!_caret_box) { _caret_box = _selection_box = _root; _caret_pos = _selection_pos = 0; }
			...
			void RichEdit::TransformSelectionIntoLink(const string & resource)
			{
				if (CanCreateLink()) {
					...
					_caret_box->ClearSelection(&_caret_pos, &_caret_box, &_selection_pos, &_selection_box);
					auto link_code = U"\33l" + resource + U"\33" + selection + U"\33e";
					Array<uint32> ucs(0x100);
			...
			void RichEdit::InsertTable(Point size)
			{
				if (CanInsertTable() && size.x > 0 && size.y > 0) {
					...
					dynamic << U"\33t" << string(uint(size.x), HexadecimalBase, 4) << string(uint(size.y), HexadecimalBase, 4);
					dynamic << U"010101" << string(uint(DefaultTextColor), HexadecimalBase, 8);
					for (int i = 0; i < size.x; i++) dynamic << U"0064";
					for (int i = 0; i < size.x * size.y; i++) dynamic << U"000000001\33e";
					dynamic << U"\33e";
