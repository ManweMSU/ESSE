...
namespace Engine
{
	namespace UI
	{
		...
		string FrameShape::ToString(void) const { return U"FrameShape"; }
		...
		string BarShape::ToString(void) const { return U"BarShape"; }
		...
		string BlurEffectShape::ToString(void) const { return U"BlurEffectShape"; }
		...
		string TextureShape::ToString(void) const { return U"TextureShape"; }
		TextShape::TextShape(const Rectangle & position, const string & text, Graphics::IFont * font, const Color & color, uint32 flags) : _atoms(0x10), _text(text), _color(color), _flags(flags)
		{
			...
			if ((_flags & TextRenderMultiline) && !(_flags & TextRenderAllowWordWrap)) {
				...
				while (pos < length) {
					...
					while (pos < length) {
						if (_text[pos] > 32) current << _text[pos];
						else if (_text[pos] == U' ') break;
						else if (_text[pos] == U'\n') { pos++; a.end_line = true; break; }
						pos++;
					}
					while (pos < length && _text[pos] == U' ') { current << _text[pos]; pos++; }
					a.text = current.ToString();
					_atoms << a;
					current.Clear();
				}
			...
			if (_atoms.Length()) {
				if (_flags & TextRenderAllowEllipsis) {
					...
					while (a.info) {
						...
						a.info = device->CreateTextBrush(_font, a.text + U"\x2026", (_flags >> 4) & 0xF, (_flags >> 8) & 0xF, _color);
					}
		...
		string TextShape::ToString(void) const { return U"TextShape"; }
		...
		string InversionEffectShape::ToString(void) const { return U"InversionEffectShape"; }
	}
}