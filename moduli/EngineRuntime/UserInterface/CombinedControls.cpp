...
namespace Engine
{
	namespace UI
	{
		namespace Controls
		{
			...
			{
				if (Template->Properties->GetTemplateClass() != U"ComboBox") throw InvalidArgumentException();
			...
			string ComboBox::GetControlClass(void) { return U"ComboBox"; }
			...
			string ComboBox::ComboListBox::GetControlClass(void) { return U"ComboListBox"; }
			...
			{
				if (Template->Properties->GetTemplateClass() != U"TextComboBox") throw InvalidArgumentException();
			...
			void TextComboBox::LeftButtonDoubleClick(Point at)
			{
				...
				while (_sp > 0 && ((IsAlphabetical(_text[_sp - 1]) || (_text[_sp - 1] >= U'0' && _text[_sp - 1] <= U'9') || (_text[_sp - 1] == U'_')))) _sp--;
				while (_cp < len && ((IsAlphabetical(_text[_cp]) || (_text[_cp] >= U'0' && _text[_cp] <= U'9') || (_text[_cp] == U'_')))) _cp++;
				find_advice();
				Invalidate();
			}
			...
			string TextComboBox::GetControlClass(void) { return U"TextComboBox"; }
			...
			string TextComboBox::TextComboListBox::GetControlClass(void) { return U"TextComboListBox"; }
		}
	}
}