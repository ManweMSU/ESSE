	...
	namespace Storage
	{
		...
		public:
			...
			void Set(const void * new_value, int size)
			{
				if (type == RegistryValueType::Integer) {
					...
					for (int i = 0; i < size; i++) {
						result += string(uint32(reinterpret_cast<const uint8 *>(new_value)[i]), U"0123456789ABCDEF", 2);
					}
					Set(result.ToString());
			...
			string GetString(void) const
			{
				if (type == RegistryValueType::Integer) return string(value_int32);
				...
				else if (type == RegistryValueType::String) return value_binary_size ? string(value_binary, -1, Encoding::UTF16) : U"";
					...
					for (uint i = 0; i < value_binary_size; i++) {
						result += string(uint32(value_binary[i]), U"0123456789ABCDEF", 2);
					}
		...
		public:
			...
			RegistryNodeImplementation * ResolveNodeByPath(const string & path)
			{
				...
				while (pos < path.Length()) {
					int ep = pos;
					while (ep < path.Length() && path[ep] != U'\\' && path[ep] != U'/') ep++;
			...
			{
				if (!name.Length() || name.FindFirst(U'\\') != -1 || name.FindFirst(U'/') != -1) return -1;
			...
			{
				auto storage = ResolveValueByPath(path);
				return storage ? storage->GetString() : U"";
			}
