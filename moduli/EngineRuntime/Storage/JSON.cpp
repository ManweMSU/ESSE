...
namespace Engine
{
	namespace Reflection
	{
		namespace JsonInternal {
			...
			{
				if (Math::IsInfinity(value) || Math::IsNaN(value)) return U"0";
				...
				for (int i = 0; i < ucs.Length(); i++) {
					if (ucs[i] < 0x20 || ucs[i] == U'\\' || ucs[i] == U'\"') {
						if (ucs[i] == U'\\') {
							...
							result += U"\\u" + string(ucs[i], HexadecimalBase, 4);
						}
					} else {
			...
			public:
				...
				void WriteSimpleProperty(PropertyInfo & pi)
				{
					if (pi.Type == PropertyType::UInt8) {
						...
						ps << U"{ \"re\": " << FormatFloatTokenJSON(v.re) << U", \"im\": " << FormatFloatTokenJSON(v.im) << U" }";
					} else if (pi.Type == PropertyType::Boolean) {
						auto v = pi.Get<bool>();
						ps << (v ? U"true" : U"false");
					} else if (pi.Type == PropertyType::String) {
						ps << U"\"" << FormatStringTokenJSON(pi.Get<string>()) << U"\"";
					} else if (pi.Type == PropertyType::Time) {
						ps << string(pi.Get<Time>());
					} else if (pi.Type == PropertyType::Color) {
						auto & v = pi.Get<Color>();
						ps << U"{ \"r\": " << string(int32(v.r)) << U", \"g\": " << string(int32(v.g)) << U", \"b\": " << string(int32(v.b)) << U", \"a\": " << string(int32(v.a)) << U" }";
					} else if (pi.Type == PropertyType::Rectangle) {
						auto & v = pi.Get<UI::Rectangle>();
						ps << U"{ \"la\": " << string(v.Left.Absolute) << U", \"ls\": " << FormatFloatTokenJSON(v.Left.Zoom) << U", \"lm\": " << FormatFloatTokenJSON(v.Left.Anchor) <<
							U", \"ta\": " << string(v.Top.Absolute) << U", \"ts\": " << FormatFloatTokenJSON(v.Top.Zoom) << U", \"tm\": " << FormatFloatTokenJSON(v.Top.Anchor) <<
							U", \"ra\": " << string(v.Right.Absolute) << U", \"rs\": " << FormatFloatTokenJSON(v.Right.Zoom) << U", \"rm\": " << FormatFloatTokenJSON(v.Right.Anchor) <<
							U", \"ba\": " << string(v.Bottom.Absolute) << U", \"bs\": " << FormatFloatTokenJSON(v.Bottom.Zoom) << U", \"bm\": " << FormatFloatTokenJSON(v.Bottom.Anchor) << U" }";
					} else {
						ps << U"null";
					}
				}
				...
				if (volume == 1) {
					if (pi.Type == PropertyType::Array) {
						if (pi.InnerType == PropertyType::UInt8) {
							...
							ps << U"[" << IO::LineFeedSequence;
							for (int i = 0; i < v.Length(); i++) { ps << *pref << U"    "; PropertyInfo spi = { v.GetBuffer() + i, inner, PropertyType::Unknown, 1, sizeof(*v.GetBuffer()), U"" }; WriteSimpleProperty(spi); if (i < v.Length() - 1) ps << U","; ps << IO::LineFeedSequence; }
							ps << *pref << U"]";
							...
							ps << U"[" << IO::LineFeedSequence;
							for (int i = 0; i < v.Length(); i++) { ps << *pref << U"    "; auto & obj = v.ElementAt(i); JsonWriteStructure(ps, *pref + U"    ", obj); if (i < v.Length() - 1) ps << U","; ps << IO::LineFeedSequence; }
							ps << *pref << U"]";
						} else {
							ps << U"null";
						}
						...
						if (pi.InnerType == PropertyType::UInt8) {
							...
							ps << U"[" << IO::LineFeedSequence;
							for (int i = 0; i < v.Length(); i++) { ps << *pref << U"    "; PropertyInfo spi = { &v[i], inner, PropertyType::Unknown, 1, sizeof(v[i]), U"" }; WriteSimpleProperty(spi); if (i < v.Length() - 1) ps << U","; ps << IO::LineFeedSequence; }
							ps << *pref << U"]";
						} else if (pi.InnerType == PropertyType::Int8) {
			...
			{
				if (node.Subnodes[0].Label == U"NUMBER") {
					bool neg = false;
					uint64 val = 0;
			...
			Math::Complex JsonAnalyzeDouble(Syntax::SyntaxTreeNode & node)
			{
				if (node.Subnodes[0].Label == U"NUMBER") {
					...
					return neg ? (-val) : val;
				} else if (node.Subnodes[0].Label == U"STRUCT") {
					ComplexWrapper val;
			...
			{
				if (node.Subnodes[0].Label == U"STRUCT") {
					ColorWrapper val;
					JsonAnalyzeStructure(val, node.Subnodes[0]);
			...
			{
				if (node.Subnodes[0].Label == U"BOOLEAN") {
					return node.Subnodes[0].Expands.AsBoolean();
				} else return false;
			}
			string JsonAnalyzeString(Syntax::SyntaxTreeNode & node)
			{
				if (node.Subnodes[0].Label == U"STRING") {
					return node.Subnodes[0].Expands.Content;
				} else return U"";
			}
#define ENUM_DYNAMIC_ARRAY_ITEMS(a) for (int i = 0; i < a.Subnodes.Length(); i++) if (a.Subnodes[i].Label == U"VALUE")
#define ENUM_STATIC_ARRAY_ITEMS(vol, a) { int i = 0, j = 0; while(j < vol && i < a.Subnodes.Length()) { if (a.Subnodes[i].Label == U"VALUE") {
...
#define GET_STATIC_ARRAY_FIELD(p) p.VolumeElement(j)
			void JsonAnalyzeStructure(Reflected & obj, Syntax::SyntaxTreeNode & node)
			{
				...
				int vi = -1;
				while (true) {
					do { ni++; } while (ni < node.Subnodes.Length() && node.Subnodes[ni].Label != U"NAME");
					do { vi++; } while (vi < node.Subnodes.Length() && node.Subnodes[vi].Label != U"VALUE");
					...
					if (pi.Address) {
						if (pi.Volume == 1) {
							if (pi.Type == PropertyType::UInt8) {
								...
								if (value.Subnodes[0].Label == U"ARRAY") {
									if (pi.InnerType == PropertyType::UInt8) {
										...
										ENUM_DYNAMIC_ARRAY_ITEMS(value.Subnodes[0]) {
											...
											if (GET_DYNAMIC_ARRAY_OBJECT(value.Subnodes[0]).Subnodes[0].Label == U"STRUCT") {
								...
								if (value.Subnodes[0].Label == U"STRUCT") JsonAnalyzeStructure(pi.Get<Reflected>(), value.Subnodes[0]);
							}
						} else {
							if (value.Subnodes[0].Label == U"ARRAY") {
								if (pi.Type == PropertyType::UInt8) {
		...
		{
			JsonInternal::JsonWriteStructure(writer, U"", obj);
		}
		void JsonSerializer::DeserializeObject(Reflected & obj)
		{
			try {
				...
				json_spelling.BooleanFalseLiteral = U"false";
				json_spelling.BooleanTrueLiteral = U"true";
				json_spelling.Keywords << U"null";
				json_spelling.IsolatedChars << U'{';
				json_spelling.IsolatedChars << U'}';
				json_spelling.IsolatedChars << U'[';
				json_spelling.IsolatedChars << U']';
				json_spelling.IsolatedChars << U',';
				...
				SafePointer<Syntax::Grammar::GrammarRule> struct_rule = new Syntax::Grammar::GrammarRule(Syntax::Grammar::GrammarRule::SequenceRule(U"STRUCT"));
				struct_rule->Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U"{"));
				struct_rule->Rules << Syntax::Grammar::GrammarRule::SequenceRule(U"", 0, 1);
				struct_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"NAME", Syntax::Token::ConstantToken(Syntax::TokenConstantClass::String));
				struct_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U":"));
				struct_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::ReferenceRule(U"", U"VALUE");
				struct_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::SequenceRule(U"", 0, -1);
				struct_rule->Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U","));
				struct_rule->Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"NAME", Syntax::Token::ConstantToken(Syntax::TokenConstantClass::String));
				struct_rule->Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U":"));
				struct_rule->Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::ReferenceRule(U"", U"VALUE");
				struct_rule->Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U"}"));
				SafePointer<Syntax::Grammar::GrammarRule> value_rule = new Syntax::Grammar::GrammarRule(Syntax::Grammar::GrammarRule::VariantRule(U"VALUE"));
				value_rule->Rules << Syntax::Grammar::GrammarRule::TokenRule(U"BOOLEAN", Syntax::Token::ConstantToken(Syntax::TokenConstantClass::Boolean));
				value_rule->Rules << Syntax::Grammar::GrammarRule::TokenRule(U"UNSET", Syntax::Token::KeywordToken(U"null"));
				value_rule->Rules << Syntax::Grammar::GrammarRule::TokenRule(U"STRING", Syntax::Token::ConstantToken(Syntax::TokenConstantClass::String));
				value_rule->Rules << Syntax::Grammar::GrammarRule::SequenceRule(U"NUMBER");
				value_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U"-"), 0, 1);
				value_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::ConstantToken(Syntax::TokenConstantClass::Numeric));
				value_rule->Rules << Syntax::Grammar::GrammarRule::ReferenceRule(U"", U"STRUCT");
				value_rule->Rules << Syntax::Grammar::GrammarRule::SequenceRule(U"ARRAY");
				value_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U"["));
				value_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::SequenceRule(U"", 0, 1);
				value_rule->Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::ReferenceRule(U"", U"VALUE");
				value_rule->Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::SequenceRule(U"", 0, -1);
				value_rule->Rules.LastElement().Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U","));
				value_rule->Rules.LastElement().Rules.LastElement().Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::ReferenceRule(U"", U"VALUE");
				value_rule->Rules.LastElement().Rules << Syntax::Grammar::GrammarRule::TokenRule(U"", Syntax::Token::CharacterToken(U"]"));
				json_grammar.Rules.Append(struct_rule->Label, struct_rule);
				json_grammar.Rules.Append(value_rule->Label, value_rule);
				json_grammar.EntranceRule = U"STRUCT";
				// Parsing
	...
	Array<uint8>* ConvertFromBase64(const string & data)
	{
		...
		if ((data.Length() & 0x3) == 0) {
			for (int i = 0; i < data.Length(); i += 4) {
				...
				uint32 number = 0;
				uint32 bytes = 3;
				for (int j = 0; j < frag.Length(); j++) if (frag[j] == U'=') bytes--;
				frag = frag.Replace(U'=', U'A');
