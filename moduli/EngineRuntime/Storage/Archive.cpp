			...
			public:
				static string NormalizeName(const string & name)
				{
					...
					while (pos < name.Length()) {
						while (name[pos] == U'\\' || name[pos] == U'/') pos++;
						while (name[pos] != U'\\' && name[pos] != U'/' && name[pos] != 0) { normalized += name[pos]; pos++; }
						if (name[pos] != 0) { normalized += U'\\'; pos++; }
					}
					return normalized;
				}
			...
			public:
				Archive(Streaming::Stream * source, ArchiveMetadataUsage usage)
				{
					...
					if (usage == ArchiveMetadataUsage::LoadMetadata) {
						if (files.LastElement().Name == U"" && files.LastElement().Type == FileTypeFromString(U"AMDE") && files.LastElement().ID == 0xFFFFFFFF && files.LastElement().Custom == 0) {
				...
				{
					if (file == 0 || file > files.Length()) throw InvalidArgumentException();
					if (metadata) return metadata->GetValueTime(string(file) + U"\\Creation Time"); else return 0;
				}
				virtual Time GetFileAccessTime(ArchiveFile file) override
				{
					if (file == 0 || file > files.Length()) throw InvalidArgumentException();
					if (metadata) return metadata->GetValueTime(string(file) + U"\\Access Time"); else return 0;
				}
				virtual Time GetFileAlterTime(ArchiveFile file) override
				{
					if (file == 0 || file > files.Length()) throw InvalidArgumentException();
					if (metadata) return metadata->GetValueTime(string(file) + U"\\Alter Time"); else return 0;
				}
				virtual bool IsFileCompressed(ArchiveFile file) override
				{
					if (file == 0 || file > files.Length()) throw InvalidArgumentException();
					if (metadata) return metadata->GetValueBoolean(string(file) + U"\\Compressed"); else return false;
				}
				virtual string GetFileAttribute(ArchiveFile file, const string & key) override
				{
					if (file == 0 || file > files.Length()) throw InvalidArgumentException();
					if (metadata) return metadata->GetValueString(string(file) + U"\\_" + key); else return U"";
				}
				virtual Array<string> * GetFileAttributes(ArchiveFile file) override
				{
					...
					if (metadata) {
						...
						if (node) {
							...
							for (int i = 0; i < names.Length(); i++) if (names[i][0] == U'_') result->Append(names[i].Fragment(1, -1));
						}
					}
					result->Retain();
					return result;
				}
				virtual string ToString(void) const override { return U"Archive"; }
			};
			class NewArchive : public Storage::NewArchive
			{
				...
				void mark_compressed(ArchiveFile file, bool flag = true)
				{
					if (metadata) {
						...
						SafePointer<RegistryNode> node = metadata->OpenNode(string(file));
						node->CreateValue(U"Compressed", RegistryValueType::Boolean);
						node->SetValue(U"Compressed", flag);
					}
				}
			...
			public:
				...
				virtual void SetFileCreationTime(ArchiveFile file, Time time) override
				{
					...
					if (metadata) {
						metadata->CreateNode(string(file));
						SafePointer<RegistryNode> node = metadata->OpenNode(string(file));
						node->CreateValue(U"Creation Time", RegistryValueType::Time);
				...
				virtual void SetFileAccessTime(ArchiveFile file, Time time) override
				{
					...
					if (metadata) {
						metadata->CreateNode(string(file));
						SafePointer<RegistryNode> node = metadata->OpenNode(string(file));
						node->CreateValue(U"Access Time", RegistryValueType::Time);
				...
				virtual void SetFileAlterTime(ArchiveFile file, Time time) override
				{
					...
					if (metadata) {
						metadata->CreateNode(string(file));
						SafePointer<RegistryNode> node = metadata->OpenNode(string(file));
						node->CreateValue(U"Alter Time", RegistryValueType::Time);
				...
				virtual void Finalize(void) override
				{
					...
					for (int i = 1; i <= GetFileCount(); i++) {
						if ((headers[i - 1].SetMask & 2) == 0) SetFileName(i, U"");
					...
					if (metadata) {
						...
						headers.LastElement().NameOffset = string_alloc(U"");
						headers.LastElement().Type = Archive::FileTypeFromString(U"AMDE");
				...
				virtual string ToString(void) const override { return U"NewArchive"; }
			};
		}
