...
namespace Engine
{
	namespace Streaming
	{
		...
		string FileStream::ToString(void) const { return U"FileStream"; }
		...
		void MemoryStream::Write(const void * _data, uint32 length)
		{
			if (length > uint32(data.Length() - pointer)) {
				...
				if (uint32(pointer) + length > 0x7FFFFFFF) throw ESSE::InputOutputException(Error::FileTooLarge);
		...
		string MemoryStream::ToString(void) const { return U"MemoryStream"; }
		...
		string TextWriter::ToString(void) const { return U"TextWriter"; }
		void ITextWriter::LineFeed(void) const { WriteLine(U""); }
		...
		void FragmentStream::Write(const void * data, uint32 length) { throw ESSE::InputOutputException(Error::IsReadOnly); }
		...
		void FragmentStream::SetLength(uint64 length) { throw ESSE::InputOutputException(Error::IsReadOnly); }
		void FragmentStream::Flush(void) {}
		FragmentStream::~FragmentStream(void) { inner->Release(); }
		string FragmentStream::ToString(void) const { return U"FragmentStream"; }
		...
		string ITextReader::ReadLine(void) const
		{
			...
			do {
				chr = ReadChar();
				if (chr != 0xFFFFFFFF && (chr >= 0x20 || chr == U'\t')) {
					result << string(&chr, 1, Encoding::UTF32);
				}
			} while (chr != 0xFFFFFFFF && chr != U'\n');
		...
		string TextReader::ToString(void) const { return U"TextReader"; }
	...
	namespace IO {
		...
		{
			string full = ExpandPath(path);
			SafePointer< Array<string> > files = Search::GetFiles(full + U"\\*");
			for (int i = 0; i < files->Length(); i++) RemoveFile(full + U"\\" + files->ElementAt(i));
			files = Search::GetDirectories(full + U"\\*");
			for (int i = 0; i < files->Length(); i++) RemoveEntireDirectory(full + U"\\" + files->ElementAt(i));
