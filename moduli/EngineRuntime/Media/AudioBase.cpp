	...
	namespace Audio
	{
		...
		StreamDesc::operator string(void) const
		{
			...
			if (Format != SampleFormat::Invalid) {
				fmt = string(SampleFormatBitSize(Format)) + string(U"-bit ");
				if (IsSignedFormat(Format)) fmt += U"signed normalized integer";
				else if (IsUnsignedFormat(Format)) fmt += U"unsigned normalized integer";
				else if (IsFloatingPointFormat(Format)) fmt += U"floating point";
			} else fmt = U"unspecified";
			if (ChannelCount) ch = string(ChannelCount); else ch = U"unspecified";
			if (FramesPerSecond) fps = string(FramesPerSecond); else fps = U"unspecified";
			return U"Stream descriptor: sample format: " + fmt + U", channels: " + ch + U", frames per second: " + fps;
		}
