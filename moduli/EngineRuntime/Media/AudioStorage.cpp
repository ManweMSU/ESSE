	...
	namespace Media
	{
		...
		class EngineMP3ContainerSink : public IMediaContainerSink
		{
				...
				if (MetadataKeyIsString(attr.key)) {
					if (MetadataKeyIsMultilineString(attr.key)) {
						...
						for (int i = 0; i < attr.value.Text.Length(); i++) {
							if (attr.value.Text[i] >= 32 || attr.value.Text[i] == U'\t' || attr.value.Text[i] == U'\n') output << attr.value.Text[i];
						}
						...
						if (attr.key == MetadataKey::Genre && output[0] == U'(') output.Insert(U"(", 0);
						if (attr.key == MetadataKey::Genre) {
							auto genre_index = metadata->GetElementByKey(MetadataKey::GenreIndex);
							if (genre_index) output.Insert(U"(" + string(genre_index->Number) + U")", 0);
						}
						SafePointer<DataBlock> encoded = output.ToString().EncodeSequence(Encoding::UTF8, false);
					...
					if (attr.key == MetadataKey::TrackNumber) {
						...
						if (frac) output += U"/" + string(frac->Number);
					...
					if (attr.key == MetadataKey::Artwork && attr.value.Picture) {
						...
						string mime_type;
						if (attr.value.Text == Codec::ImageFormatPNG) mime_type = U"image/png";
						else mime_type = U"image/jpeg";
			...
			public:
				...
				virtual string GetTrackName(void) const override { return U""; }
				virtual string GetTrackLanguage(void) const override { return U""; }
				...
				while (pos <= data_size - hdr_size) {
					...
					if ((hdr_v2 && hdr_v2->name[0] == 'T') || (hdr_v3v4 && hdr_v3v4->name[0] == 'T')) {
						...
						if (!skip) {
							...
							try {
								if (code == U"TALB" || code == U"TAU") metadata.Append(MetadataKey::Album, string);
								else if (code == U"TBPM" || code == U"TBP") metadata.Append(MetadataKey::BeatsPerMinute, string.ToUInt32());
								else if (code == U"TCOM" || code == U"TCM") metadata.Append(MetadataKey::Composer, string);
								else if (code == U"TCOP" || code == U"TCR") metadata.Append(MetadataKey::Copyright, string);
								else if (code == U"TENC" || code == U"TEN") metadata.Append(MetadataKey::Encoder, string);
								else if (code == U"TIT2" || code == U"TT2") metadata.Append(MetadataKey::Title, string);
								else if (code == U"TIT3" || code == U"TT3") metadata.Append(MetadataKey::Description, string);
								else if (code == U"TYER" || code == U"TYE") metadata.Append(MetadataKey::Year, string);
								else if (code == U"TPE1" || code == U"TP1") metadata.Append(MetadataKey::Artist, string);
								else if (code == U"TPE2" || code == U"TP2") metadata.Append(MetadataKey::AlbumArtist, string);
								else if (code == U"TPUB") metadata.Append(MetadataKey::Publisher, string);
								else if (code == U"TCON" || code == U"TCO") {
									DynamicString str;
									int pos = 0;
									while (string[pos] == U'(') {
										pos++;
										if (string[pos] == U'(') break;
										int sp = pos;
										while (pos < string.Length() && string[pos] != U')') pos++;
										try {
								...
								} else if (code == U"TRCK" || code == U"TRK") {
									int index;
									if ((index = string.FindFirst(U'/')) != -1) {
								...
								} else if (code == U"TPOS" || code == U"TPA") {
									int index;
									if ((index = string.FindFirst(U'/')) != -1) {
						...
						try {
							...
							if (type == U"image/png") pic_fmt = Codec::ImageFormatPNG;
		...
		class EngineFLACContainerSource : public IMediaContainerSource
		{
			static void _read_core_metadata(Streaming::Stream * stream, Metadata * metadata)
			{
				...
				for (uint i = 0; i < num_tags; i++) {
					...
					int index = tag.FindFirst(U'=');
					if (index < 0) continue;
					string key = tag.Fragment(0, index);
					string value = tag.Fragment(index + 1, -1);
					try {
						if (key == U"TITLE") metadata->Append(MetadataKey::Title, value);
						else if (key == U"ALBUM") metadata->Append(MetadataKey::Album, value);
						else if (key == U"TRACKNUMBER") metadata->Append(MetadataKey::TrackNumber, value.ToUInt32());
						else if (key == U"DISCNUMBER") metadata->Append(MetadataKey::DiskNumber, value.ToUInt32());
						else if (key == U"TRACKTOTAU" || key == U"TOTALTRACKS") metadata->Append(MetadataKey::TrackCount, value.ToUInt32());
						else if (key == U"DISCTOTAU" || key == U"TOTALDISCS") metadata->Append(MetadataKey::DiskCount, value.ToUInt32());
						else if (key == U"ARTIST") metadata->Append(MetadataKey::Artist, value);
						else if (key == U"ALBUMARTIST" || key == U"BAND") metadata->Append(MetadataKey::AlbumArtist, value);
						else if (key == U"COPYRIGHT") metadata->Append(MetadataKey::Copyright, value);
						else if (key == U"DESCRIPTION") metadata->Append(MetadataKey::Description, value);
						else if (key == U"COMMENT") metadata->Append(MetadataKey::Comment, value);
						else if (key == U"GENRE") metadata->Append(MetadataKey::Genre, value);
						else if (key == U"DATE") metadata->Append(MetadataKey::Year, value);
						else if (key == U"COMPOSER") metadata->Append(MetadataKey::Composer, value);
						else if (key == U"LYRICS") metadata->Append(MetadataKey::Lyrics, value);
						else if (key == U"PUBLISHER") metadata->Append(MetadataKey::Publisher, value);
					} catch (...) {}
				}
			}
			static void _read_artwork_metadata(Streaming::Stream * stream, Metadata * metadata)
			{
				...
				string format = (mime == U"image/png") ? Codec::ImageFormatPNG : Codec::ImageFormatJPEG;
			...
			static DataBlock * _create_core_metadata_block(const Metadata * metadata, bool encode_as_final)
			{
				...
				for (auto & tag : metadata->Elements()) {
					string tag_encode;
					if (tag.key == MetadataKey::Album) tag_encode = U"ALBUM=" + tag.value.Text;
					else if (tag.key == MetadataKey::AlbumArtist) tag_encode = U"ALBUMARTIST=" + tag.value.Text;
					else if (tag.key == MetadataKey::Artist) tag_encode = U"ARTIST=" + tag.value.Text;
					else if (tag.key == MetadataKey::Comment) tag_encode = U"COMMENT=" + tag.value.Text;
					else if (tag.key == MetadataKey::Composer) tag_encode = U"COMPOSER=" + tag.value.Text;
					else if (tag.key == MetadataKey::Copyright) tag_encode = U"COPYRIGHT=" + tag.value.Text;
					else if (tag.key == MetadataKey::Description) tag_encode = U"DESCRIPTION=" + tag.value.Text;
					else if (tag.key == MetadataKey::Genre) tag_encode = U"GENRE=" + tag.value.Text;
					else if (tag.key == MetadataKey::Lyrics) tag_encode = U"LYRICS=" + tag.value.Text;
					else if (tag.key == MetadataKey::Publisher) tag_encode = U"PUBLISHER=" + tag.value.Text;
					else if (tag.key == MetadataKey::Title) tag_encode = U"TITLE=" + tag.value.Text;
					else if (tag.key == MetadataKey::Year) tag_encode = U"DATE=" + tag.value.Text;
					else if (tag.key == MetadataKey::TrackNumber) tag_encode = U"TRACKNUMBER=" + string(tag.value.Number);
					else if (tag.key == MetadataKey::DiskNumber) tag_encode = U"DISCNUMBER=" + string(tag.value.Number);
					else if (tag.key == MetadataKey::TrackCount) tag_encode = U"TRACKTOTAL=" + string(tag.value.Number);
					else if (tag.key == MetadataKey::DiskCount) tag_encode = U"DISCTOTAL=" + string(tag.value.Number);
		...
		public:
			...
			virtual string GetCodecName(void) const override { return U"Engine MPEG-1/2 Part 3 and FLAC Multiplexor"; }
