	...
	namespace Media
	{
		...
		string AudioTrackFormatDesc::ToString(void) const
		{
			...
			if (_desc.Format != Audio::SampleFormat::Invalid) {
				fmt = string(Audio::SampleFormatBitSize(_desc.Format)) + string(U"-bit ");
				if (Audio::IsSignedFormat(_desc.Format)) fmt += U"signed normalized integer";
				else if (Audio::IsUnsignedFormat(_desc.Format)) fmt += U"unsigned normalized integer";
				else if (Audio::IsFloatingPointFormat(_desc.Format)) fmt += U"floating point";
			} else fmt = U"unspecified";
			if (_desc.ChannelCount) ch = string(_desc.ChannelCount); else ch = U"unspecified";
			if (_desc.FramesPerSecond) fps = string(_desc.FramesPerSecond); else fps = U"unspecified";
			if (_channel_layout) return FormatString(U"Audio track: codec: %0, sample format: %1, channel count: %2, channel layout: %3, frames per second: %4", _codec, fmt, ch, string(_channel_layout, HexadecimalBase, 8), fps);
			else return FormatString(U"Audio track: codec: %0, sample format: %1, channel count: %2, channel layout: unspecified, frame rate: %3", _codec, fmt, ch, fps);
		}
		...
		{
			if (_pixel_aspect_horz > 1 && _pixel_aspect_vert > 1) return FormatString(U"Video track: codec: %0, resolution: %1 x %2, frame rate: %3/%4, pixel aspect ratio: %5/%6",
				_codec, _width, _height, _rate_numerator, _rate_denominator, _pixel_aspect_horz, _pixel_aspect_vert);
			else return FormatString(U"Video track: codec: %0, resolution: %1 x %2, frame rate: %3/%4", _codec, _width, _height, _rate_numerator, _rate_denominator);
		}
		...
		{
			return FormatString(U"Subtitle track: codec: %0, frame rate: %1, flags: %2", _codec, _time_scale, string(_flags, BinaryBase, 4));
		}
		...
		ContainerClassDesc::operator string(void) const
		{
			...
			result << U"Container format: " << ContainerFormatIdentifier;
			result << U", audio supported: " << (FormatCapabilities & ContainerClassCapabilityHoldAudio ? U"yes" : U"no");
			result << U", video supported: " << (FormatCapabilities & ContainerClassCapabilityHoldVideo ? U"yes" : U"no");
			result << U", subtitles supported: " << (FormatCapabilities & ContainerClassCapabilityHoldSubtitles ? U"yes" : U"no");
			result << U", metadata supported: " << (FormatCapabilities & ContainerClassCapabilityHoldMetadata ? string(U"yes, ") + MetadataFormatIdentifier : string(U"no"));
			result << U", interleaved tracks supported: " << (FormatCapabilities & ContainerClassCapabilityInterleavedTracks ? U"yes" : U"no");
			result << U", key frames supported: " << (FormatCapabilities & ContainerClassCapabilityKeyFrames ? U"yes" : U"no");
			result << U", maximal track count: " << (MaximalTrackCount ? string(MaximalTrackCount) : string(U"unlimited"));
