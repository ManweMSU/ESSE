...
namespace Engine
{
	namespace Media
	{
		...
		struct Atom : public Object
		{
			...
			{
				return fcc == U"moov" || fcc == U"trak" || fcc == U"tref" || fcc == U"mdia" || fcc == U"minf" || fcc == U"stbl" || fcc == U"udta" || fcc == U"ilst";
			}
			static Atom * ReadAtom(Streaming::Stream * from, uint64 upper_bound, bool top_level_atom = false)
			{
				...
				try {
					...
					if (top_level_atom && result->fourcc != U"ftyp" && result->fourcc != U"moov") {
		...
		void iTunesWriteStringAttribute(Atom * outer, const string & name, const string & text)
		{
			...
			SafePointer<Atom> data = new Atom(U"data");
		...
		{
			SafePointer<Atom> udta = new Atom(U"udta");
			SafePointer<Atom> meta = new Atom(U"meta");
			udta->children.Append(meta);
			uint zero = 0;
			meta->Write(&zero, 4);
			SafePointer<Atom> hdlr = new Atom(U"hdlr");
			hdlr->Write(&zero, 4);
			hdlr->Write(&zero, 4);
			...
			hdlr->Write(&zero, 1);
			meta->Write(hdlr);
			SafePointer<Atom> ilst = new Atom(U"ilst");
			for (auto & meta : metadata->Elements()) {
				if (meta.key == MetadataKey::Title) iTunesWriteStringAttribute(ilst, U"\251nam", meta.value.Text);
				else if (meta.key == MetadataKey::Album) iTunesWriteStringAttribute(ilst, U"\251alb", meta.value.Text);
				else if (meta.key == MetadataKey::Artist) iTunesWriteStringAttribute(ilst, U"\251art", meta.value.Text);
				else if (meta.key == MetadataKey::AlbumArtist) {
					iTunesWriteStringAttribute(ilst, U"aART", meta.value.Text);
					iTunesWriteStringAttribute(ilst, U"\251ART", meta.value.Text);
				} else if (meta.key == MetadataKey::Year) iTunesWriteStringAttribute(ilst, U"\251day", meta.value.Text);
				else if (meta.key == MetadataKey::Genre) iTunesWriteStringAttribute(ilst, U"\251gen", meta.value.Text);
				else if (meta.key == MetadataKey::Composer) iTunesWriteStringAttribute(ilst, U"\251wrt", meta.value.Text);
				else if (meta.key == MetadataKey::Encoder) iTunesWriteStringAttribute(ilst, U"\251too", meta.value.Text);
				else if (meta.key == MetadataKey::Copyright) iTunesWriteStringAttribute(ilst, U"cprt", meta.value.Text);
				else if (meta.key == MetadataKey::Description) iTunesWriteStringAttribute(ilst, U"desc", meta.value.Text);
				else if (meta.key == MetadataKey::Comment) iTunesWriteStringAttribute(ilst, U"\251cmt", meta.value.Text);
				else if (meta.key == MetadataKey::Lyrics) iTunesWriteStringAttribute(ilst, U"\251lyr", meta.value.Text);
				else if (meta.key == MetadataKey::TrackNumber) {
					auto count = metadata->GetElementByKey(MetadataKey::TrackCount);
					iTunesWritePairedAttribute(ilst, U"trkn", meta.value.Number, count ? count->Number : 0);
				} else if (meta.key == MetadataKey::DiskNumber) {
					auto count = metadata->GetElementByKey(MetadataKey::DiskCount);
					iTunesWritePairedAttribute(ilst, U"disk", meta.value.Number, count ? count->Number : 0);
				} else if (meta.key == MetadataKey::BeatsPerMinute) iTunesWriteNumericAttribute(ilst, U"tmpo", meta.value.Number, 0x15000000);
				else if (meta.key == MetadataKey::GenreIndex) iTunesWriteNumericAttribute(ilst, U"gnre", meta.value.Number + 1, 0);
					...
					if (format_code) iTunesWritePictureAttribute(ilst, U"covr", meta.value.Picture, format_code);
				}
			}
			meta->Write(ilst);
			udta->Retain();
		...
		class MPEG4TrackSink : public IMediaTrackSink
		{
			...
			void _make_stsd_for_video(Atom * stsd)
			{
				...
				if (vd.GetTrackCodec() == Video::VideoFormatH264) {
					...
					uint avcC_fourcc = MakeFourCC(U"avcC");
					...
					uint pasp_fourcc = MakeFourCC(U"pasp");
					...
					uint atom_fourcc = MakeFourCC(U"pasp");
					uint pasp_horz = Network::InverseEndianess(uint(vd.GetPixelAspectHorizontal()));
					uint pasp_vert = Network::InverseEndianess(uint(vd.GetPixelAspectVertical()));
			...
			{
				SafePointer<Atom> stsd = new Atom(U"stsd");
			...
			void _init_stbl(void)
			{
				...
				SafePointer<Atom> stts = new Atom(U"stts", 8 + 8 * _packets.Length());
				...
				SafePointer<Atom> stsz = new Atom(U"stsz", uniform_size ? 12 : (12 + 4 * _packets.Length()));
				...
				SafePointer<Atom> stsc = new Atom(U"stsc", 20);
				...
				if (_packets.LastElement().file_offset <= 0xD0000000) {
					SafePointer<Atom> stco = new Atom(U"stco", 8 + 4 * _packets.Length());
					...
					SafePointer<Atom> co64 = new Atom(U"co64", 8 + 8 * _packets.Length());
				...
				if (has_non_key_frames) {
					SafePointer<Atom> stss = new Atom(U"stss", 8 + 4 * num_key_frames);
				...
				if (has_shifts) {
					uint num_records = 0;
					SafePointer<Atom> ctts = new Atom(U"ctts", 8 + 8 * _packets.Length());
			...
			{
				minf = new Atom(U"minf");
				hdlr = new Atom(U"hdlr", sizeof(Format::mpeg4_hdlr));
				auto & hdlr_hdr = hdlr->Interpret<Format::mpeg4_hdlr>();
				ZeroMemory(&hdlr_hdr, sizeof(hdlr_hdr));
				if (_desc->GetTrackClass() == TrackClass::Audio) {
					auto & ad = _desc->As<AudioTrackFormatDesc>();
					SafePointer<Atom> smhd = new Atom(U"smhd", sizeof(Format::mpeg4_smhd));
					...
					auto & vd = _desc->As<VideoTrackFormatDesc>();
					SafePointer<Atom> vmhd = new Atom(U"vmhd", sizeof(Format::mpeg4_vmhd));
					...
					auto & sd = _desc->As<SubtitleTrackFormatDesc>();
					SafePointer<Atom> nmhd = new Atom(U"nmhd");
					uint write = 0;
					nmhd->Write(&write, 4);
				...
				SafePointer<Atom> dinf = new Atom(U"dinf");
				SafePointer<Atom> dref = new Atom(U"dref");
				SafePointer<Atom> data_hdlr = new Atom(U"hdlr", sizeof(Format::mpeg4_hdlr));
			...
			{
				mdia = new Atom(U"mdia");
				SafePointer<Atom> mdhd = new Atom(U"mdhd", sizeof(Format::mpeg4_mdhd));
			...
			{
				trak = new Atom(U"trak");
				if (!_visible) {
					SafePointer<Atom> txas = new Atom(U"txas");
				...
				if (_name.Length()) {
					...
					SafePointer<Atom> udta = new Atom(U"udta");
					SafePointer<Atom> name = new Atom(U"name");
					SafePointer<Atom> titl = new Atom(U"titl");
		...
		public:
			...
			virtual bool Finalize(void) noexcept override
			{
				...
				try {
					...
					moov = new Atom(U"moov");
					SafePointer<Atom> mvhd = new Atom(U"mvhd", sizeof(Format::mpeg4_mvhd));
					...
					for (int i = 0; i < _tracks.Length(); i++) {
						...
						SafePointer<Atom> tkhd = new Atom(U"tkhd", sizeof(Format::mpeg4_tkhd));
						...
						if (track._desc->GetTrackClass() == TrackClass::Audio) {
							...
							if (track.stbl) {
								auto & ssd = *reinterpret_cast<Format::mpeg4_subtitle_sample_desc *>(track.stbl->FindPrimaryChild(U"stsd")->data->GetBuffer() + 8);
						...
						SafePointer<Atom> edts = new Atom(U"edts");
						SafePointer<Atom> elst = new Atom(U"elst");
						uint version = 0;
		...
		class MPEG4TrackSource : public IMediaTrackSource
		{
			...
			void _load_video_desc(Atom * stsd, Format::mpeg4_stts & stts_table)
			{
				...
				while (size - offset >= 8) {
					...
					if (ex_fourcc == U"pasp" && size - offset >= 16) {
					...
					} else if (ex_fourcc == U"esds" && size - offset >= 12) {
						int len = ex_size - 12;
					...
					} else if (ex_fourcc == U"avcC" && size - offset >= 8) {
						int len = ex_size - 8;
						_codec_usage_data = new DataBlock(len);
						_codec_usage_data->SetLength(len);
			...
			{
				auto stts = stbl->FindPrimaryChild(U"stts");
				if (!stts) throw InvalidFormatException();
				auto stco = stbl->FindPrimaryChild(U"stco");
				auto co64 = stbl->FindPrimaryChild(U"co64");
				if (!stco && !co64) throw InvalidFormatException();
				auto stsc = stbl->FindPrimaryChild(U"stsc");
				if (!stsc) throw InvalidFormatException();
				auto stsz = stbl->FindPrimaryChild(U"stsz");
				if (!stsz) throw InvalidFormatException();
				auto stss = stbl->FindPrimaryChild(U"stss");
				auto sdtp = stbl->FindPrimaryChild(U"sdtp");
				auto ctts = stbl->FindPrimaryChild(U"ctts");
				int chunk_count;
			...
			{
				auto mdhd = mdia->FindPrimaryChild(U"mdhd");
				...
				auto hdlr = mdia->FindPrimaryChild(U"hdlr");
				auto minf = mdia->FindPrimaryChild(U"minf");
				...
				auto stbl = minf->FindPrimaryChild(U"stbl");
				if (!stbl) throw InvalidFormatException();
				_load_packet_layout(stbl);
				auto stsd = stbl->FindPrimaryChild(U"stsd");
		...
		public:
			MPEG4TrackSource(Streaming::Stream * stream, IMediaContainerSource * source, Atom * trak, uint32 common_time_scale) : _packets(0x10000)
			{
				...
				auto tkhd = trak->FindPrimaryChild(U"tkhd");
				...
				if (trak->FindPrimaryChild(U"txas")) _autoselectable = false; else _autoselectable = true;
				auto udta = trak->FindPrimaryChild(U"udta");
				if (udta) {
					auto name = udta->FindPrimaryChild(U"name");
					if (name) _name = string(name->data->GetBuffer(), name->data->Length(), Encoding::UTF8);
				}
				auto mdia = trak->FindPrimaryChild(U"mdia");
		...
		class MPEG4Source : public IMediaContainerSource
		{
			...
			{
				number = count = 0;
				for (auto & atom : root->children) if (atom.fourcc == U"data") {
			...
			static Metadata * _read_metadata(Atom * ilst)
			{
				...
				for (auto & atom : ilst->children) {
					...
					} else if (atom.fourcc == U"disk") {
						uint32 no, cnt; _read_track_number(&atom, no, cnt);
						if (no) result->Append(MetadataKey::DiskNumber, no);
						if (cnt) result->Append(MetadataKey::DiskCount, cnt);
					} else if (atom.fourcc == U"\251alb") result->Append(MetadataKey::Album, _read_string(&atom));
					else if (atom.fourcc == U"\251art") result->Append(MetadataKey::Artist, _read_string(&atom));
					else if (atom.fourcc == U"\251ART") result->Append(MetadataKey::Artist, _read_string(&atom));
					else if (atom.fourcc == U"aART") result->Append(MetadataKey::AlbumArtist, _read_string(&atom));
					else if (atom.fourcc == U"\251cmt") result->Append(MetadataKey::Comment, _read_string(&atom));
					else if (atom.fourcc == U"\251day") result->Append(MetadataKey::Year, _read_string(&atom).Fragment(0, 4));
					else if (atom.fourcc == U"\251nam") result->Append(MetadataKey::Title, _read_string(&atom));
					else if (atom.fourcc == U"\251gen") result->Append(MetadataKey::Genre, _read_string(&atom));
					else if (atom.fourcc == U"\251wrt") result->Append(MetadataKey::Composer, _read_string(&atom));
					else if (atom.fourcc == U"\251too") result->Append(MetadataKey::Encoder, _read_string(&atom));
					else if (atom.fourcc == U"cprt") result->Append(MetadataKey::Copyright, _read_string(&atom));
					else if (atom.fourcc == U"desc") result->Append(MetadataKey::Description, _read_string(&atom));
					else if (atom.fourcc == U"\251lyr") result->Append(MetadataKey::Lyrics, _read_string(&atom));
					else if (atom.fourcc == U"tmpo") result->Append(MetadataKey::BeatsPerMinute, _read_number(&atom));
					else if (atom.fourcc == U"gnre") result->Append(MetadataKey::GenreIndex, _read_number(&atom) - 1);
					else if (atom.fourcc == U"covr") {
						string format;
		...
		public:
			MPEG4Source(Streaming::Stream * source, IMediaContainerCodec * codec, const ContainerClassDesc & desc) : _desc(desc)
			{
				...
				while (atom = Atom::ReadAtom(_source, size, true)) {
					if (atom->fourcc == U"ftyp" && !_ftyp) _ftyp.SetRetain(atom);
					else if (atom->fourcc == U"moov" && !_moov) _moov.SetRetain(atom);
					else if (atom->fourcc == U"mdat" && !_mdat) _mdat.SetRetain(atom);
					atom->Release();
				}
				if (!_ftyp || !_moov || !_mdat) throw InvalidFormatException();
				auto mvhd = _moov->FindPrimaryChild(U"mvhd");
				...
				for (auto & a : _moov->children) if (a.fourcc == U"trak") {
					try {
				...
				auto udta = _moov->FindPrimaryChild(U"udta");
				auto meta = udta ? udta->FindPrimaryChild(U"meta") : 0;
			...
			virtual Metadata * ReadMetadata(void) const noexcept override
			{
				if (_itunes_md_holder) {
					try {
						...
						auto ilst = atom->FindPrimaryChild(U"ilst");
						if (!ilst) return 0;
			...
			{
				try {
					for (int i = 0; i < _moov->children.Length(); i++) if (_moov->children[i].fourcc == U"udta") {
			...
			virtual bool PatchMetadata(const Metadata * metadata, Streaming::Stream * dest) const noexcept override
			{
				try {
					...
					for (int i = 0; i < moov->children.Length(); i++) if (moov->children[i].fourcc == U"udta") {
				...
				try {
					if (packet.PacketDataActuallyUsed) {
						...
						while (ext_read_pos + 8 <= packet.PacketDataActuallyUsed) {
							...
							} else if (fcc == U"twrp" && ext_read_pos + 9 <= packet.PacketDataActuallyUsed) {
		...
		public:
			...
			virtual string GetCodecName(void) const override { return U"Engine MPEG-4 Multiplexor"; }
		...
		public:
			...
			virtual string GetCodecName(void) const override { return U"Engine 3GPP Timed Text Codec"; }
