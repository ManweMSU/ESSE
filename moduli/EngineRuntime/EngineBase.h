...
namespace Engine
{
	...
	public:
		...
		ImmutableString(const wchar_t * src);
		...
		ImmutableString(float src, widechar separator = U'.', int digits = 7);
		ImmutableString(double src, widechar separator = U'.', int digits = 16);
		...
		ImmutableString & operator = (const char * src);
		ImmutableString & operator = (const wchar_t * src);
		...
		ImmutableString friend operator + (const wchar_t * a, const ImmutableString & b);
		ImmutableString friend operator + (const ImmutableString & a, const wchar_t * b);
	...
	constexpr const widechar * DecimalBase = U"0123456789";
	constexpr const widechar * HexadecimalBase = U"0123456789ABCDEF";
	constexpr const widechar * HexadecimalBaseLowerCase = U"0123456789abcdef";
	constexpr const widechar * OctalBase = U"01234567";
	constexpr const widechar * BinaryBase = U"01";
	...
	template<class T> std::enable_if_t<!std::is_convertible<T, ImmutableString>::value && !HasToStringTraits<T>::value, ImmutableString> GetStringRepresentation(const T & t) { return U"?"; }
	...
	public:
		...
		string ToString(void) const override
		{
			string result = U"Object Array : [";
			if (count) for (int i = 0; i < count; i++) { result += data[i]->ToString() + ((i == count - 1) ? U"]" : U", "); }
			else result += U"]";
			return result;
		}
