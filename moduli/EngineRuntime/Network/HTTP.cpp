		...
		namespace HTTP
		{
			class HttpRequest : public Network::HttpRequest
			{
				...
				public:
							...
							while (length) {
								if (!current_chunk) {
									try {
										...
										read_string();
									} catch (...) { throw ESSE::InputOutputException(ESSE::Errores::SuberrorIO::Unknown); }
								}
					...
					{
						if (!max_length_set) throw ESSE::InputOutputException(ESSE::Errores::SuberrorIO::Unknown);
				...
				void send_http_header(void)
				{
					...
					write_string(encoded, _verb + U" " + _uri + U" HTTP/1.1");
					write_newline(encoded);
					for (int i = 0; i < _headers.Length(); i++) {
						write_string(encoded, _headers[i].Name + U": " + _headers[i].Content);
						write_newline(encoded);
					}
					write_newline(encoded);
					...
					while (true) {
						...
						if (resp.Length()) {
							if (first) {
								first = false;
								int sp1 = 0, sp2 = 0;
								while (resp[sp1] && resp[sp1] != U' ') sp1++;
								sp2 = sp1 + 1;
								while (resp[sp2] && resp[sp2] != U' ') sp2++;
								_status = resp.Fragment(sp1 + 1, sp2 - sp1 - 1).ToUInt32();
							} else {
								int cp = 0;
								while (resp[cp] && resp[cp] != U':') cp++;
								int bp = cp + 1;
								while (resp[bp] && resp[bp] == U' ') bp++;
								int ep = resp.Length() - 1;
								while (ep && resp[ep] == U' ') ep--;
								SetHeader(resp.Fragment(0, cp), (ep >= bp) ? resp.Fragment(bp, ep - bp + 1) : U"");
							}
						} else break;
					}
					string len = GetHeader(U"Content-Length");
					string con_enc = GetHeader(U"Transfer-Encoding");
					if (len.Length()) {
						reader = new ReaderStream(_connection, len.ToUInt32());
					} else if (string::CompareIgnoreCase(con_enc, U"chunked") == 0) {
			...
			public:
				HttpRequest(const string & agent, const string & host, const string & uri, const widechar * verb, HttpVerb everb, Socket * connection) : _agent(agent), _host(host), _uri(uri), _verb(verb), _headers(0x10)
				{
					...
					SetHeader(U"Cache-Control", U"no-cache");
					SetHeader(U"Accept", U"*");
					SetHeader(U"Accept-Charset", U"utf-8");
					SetHeader(U"Host", _host);
					SetHeader(U"Connection", U"close");
					if (_agent.Length()) SetHeader(U"User-Agent", _agent);
				}
				...
				{
					if (_status) throw InvalidStateException();
					if (size) SetHeader(U"Content-Length", string(size));
					send_http_header();
				...
				virtual Streaming::Stream * BeginSend(uint32 size) override
				{
					...
					SetHeader(U"Content-Length", string(size));
					send_http_header();
			...
			public:
				...
				virtual HttpRequest * CreateRequest(const string & object, HttpVerb verb) override
				{
					...
					const widechar * Verb = U"GET";
					if (verb == HttpVerb::Options) Verb = U"OPTIONS";
					else if (verb == HttpVerb::Get) Verb = U"GET";
					else if (verb == HttpVerb::Head) Verb = U"HEAD";
					else if (verb == HttpVerb::Post) Verb = U"POST";
					else if (verb == HttpVerb::Put) Verb = U"PUT";
					else if (verb == HttpVerb::Patch) Verb = U"PATCH";
					else if (verb == HttpVerb::Delete) Verb = U"DELETE";
					else if (verb == HttpVerb::Trace) Verb = U"TRACE";
					else if (verb == HttpVerb::Connect) Verb = U"CONNECT";
					try {
				...
				virtual void ResetConnection(void) override
				{
					...
					if (_host[0] == U'[') {
						int sep;
						if ((sep = _host.FindLast(U':')) != -1 && sep > _host.FindLast(U']')) {
