...
namespace Engine
{
	...
	{
		Access->Wait();
		string info = U"Thread Pool: " + string(Workers.Length()) + U" threads, " + string(NumTasks) + U" tasks pending, " + string(ActiveThreads) + U" threads active";
		Access->Open();
		return info;
	}
	...
	{
		Access->Wait();
		string info = U"Task Queue: " + string(IntTaskCount) + U" tasks pending";
		Access->Open();
		return info;
	}
