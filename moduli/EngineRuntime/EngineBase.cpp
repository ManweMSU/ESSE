...
namespace Engine
{
	namespace SymbolicMath
	{
		...
		{
			int dot1 = a.FindFirst(U'.');
			int dot2 = b.FindFirst(U'.');
			string a1 = (dot1 < dot2) ? string(U'0', dot2 - dot1) + a : a;
			string b1 = (dot2 < dot1) ? string(U'0', dot1 - dot2) + b : b;
			if (a1.Length() > b1.Length()) b1 += string(U'0', a1.Length() - b1.Length());
			else a1 += string(U'0', b1.Length() - a1.Length());
			...
			for (int i = a1.Length() - 1; i >= 0; i--) {
				if (a1[i] == U'.') {
					...
					int summ = int(a1[i] - U'0') + int(b1[i] - U'0') + carry;
					s1[i + 1] = (summ % 10) + U'0';
					carry = summ / 10;
				}
			}
			s1[0] = U'0' + carry;
			try {
				string s = (s1[0] == U'0') ? string(s1 + 1, a1.Length()) : string(s1, a1.Length() + 1);
				delete[] s1; s1 = 0;
				return s;
			}
		...
		string DivideByTwo(const string & a)
		{
			...
			for (int i = 0; i < a.Length(); i++) {
				if (a[i] == U'.') {
					...
					int value = carry * 10 + int(a[i] - U'0');
					s1[i] = U'0' + (value / 2);
					carry = value % 2;
				}
			}
			if (carry) s1[a.Length()] = U'5';
			try {
		...
		string TwoPower(int power)
		{
			if (power > 0) {
				...
				if (!s1) throw OutOfMemoryException();
				for (int i = 0; i < len - 3; i++) s1[i] = U'0';
				s1[len - 3] = U'1'; s1[len - 2] = U'.'; s1[len - 1] = U'0';
				int carry, nsig = len - 3;
				for (int j = 0; j < power; j++) {
					carry = 0;
					for (int i = len - 3; i >= 0; i--) {
						int value = int(s1[i] - U'0') * 2 + carry;
						s1[i] = U'0' + (value % 10);
						carry = value / 10;
		...
		{
			int fnz = 0;
			while (a[fnz] == U'0' || a[fnz] == U'.') fnz++;
			int truncf = fnz;
			for (int i = 0; i < signchars; i++) {
				truncf++;
				if (truncf >= a.Length()) break;
				if (a[truncf] == U'.') truncf++;
			}
			int len = truncf + 1;
			int dot = a.FindFirst(U'.');
			int ll;
			if (truncf > dot) ll = len; else ll = len + (dot - truncf) + 1;
			widechar * s1 = new (std::nothrow) widechar[ll];
			if (!s1) throw OutOfMemoryException();
			for (int i = len; i < ll; i++) s1[i] = U'0';
			if (len != ll) s1[ll - 1] = U'.';
			int carry = ((truncf < a.Length()) && (a[truncf] >= U'5')) ? 1 : 0;
			if (carry) {
				for (int i = len - 1; i >= 1; i--) {
					if (a[i - 1] == U'.') {
						s1[i] = U'.';
					} else {
						int value = int(a[i - 1] - U'0') + carry;
						s1[i] = U'0' + (value % 10);
						carry = value / 10;
					}
				}
			...
			int begin = 0;
			while (s1[begin] == U'0') begin++;
			if (s1[begin] == U'.') begin--;
			int len2 = ll - 1;
			while (s1[len2] == U'0') len2--;
			if (s1[len2] == U'.') len2--;
			for (int i = 0; i < ll; i++) if (s1[i] == U'.') { s1[i] = separator; break; }
			try {
	...
	ImmutableString::ImmutableString(const wchar_t * src) : ImmutableString(src, -1, LegacySystemEncoding) {}
	...
	ImmutableString::ImmutableString(int32 src)
	{
		...
		if (src == 0x80000000) {
			...
			do {
				int r = src % 10;
				src /= 10;
				StringAppend(conv, U"0123456789"[r]);
			} while (src);
			if (neg) StringAppend(conv, U'-');
		}
	...
	ImmutableString::ImmutableString(int64 src)
	{
		...
		if (src == 0x8000000000000000) {
			rev = false;
			StringCopy(conv, U"-9223372036854775808");
		} else {
			bool neg = false;
			if (src < 0) {
				src = -src; neg = true;
	...
	ImmutableString::ImmutableString(const void * src) : ImmutableString(intptr(src), U"0123456789ABCDEF", sizeof(void*) * 2) {}
	...
	ImmutableString::ImmutableString(float src, widechar separator, int digits) : ImmutableString()
	{
		...
		if (exp == 0) {
			if (value == 0) *this = U"0";
				...
				*this = (negative) ? (U"-" + base) : base;
			}
		} else if (exp == 0xFF) {
			if (value == 0) *this = (negative) ? U"-\x221E" : U"+\x221E";
			else *this = U"NaN";
		} else {
			exp -= 127;
	...
	ImmutableString::ImmutableString(bool src) : ImmutableString(src ? U"true" : U"false") {}
	...
	ImmutableString::ImmutableString(const Object * object) : ImmutableString(object ? object->ToString() : string(U"nulU")) {}
	...
	ImmutableString & ImmutableString::operator=(const char * src) { return *this = string(src); }
	ImmutableString & ImmutableString::operator=(const wchar_t * src) { return *this = string(src); }
	ImmutableString::operator const widechar*(void) const { return text ? text : U""; }
	...
	ImmutableString ImmutableString::Fragment(int PosStart, int CharLength) const
	{
		if (!text) return U"";
		int len = Length();
		if (PosStart < 0 || PosStart > len || !CharLength) return U"";
		if (CharLength < -1) return U"";
	...
	uint64 ImmutableString::GeneralizedToUInt64(int dfrom, int dto) const
	{
		...
		for (int i = dfrom; i < dto; i++) {
			if (text[i] < U'0' || text[i] > U'9') throw InvalidFormatException();
			uint64 digit = text[i] - U'0';
	...
	ImmutableString ImmutableString::LowerCase(void) const { if (!text) return U""; ImmutableString result = *this; StringLower(result.text, Length()); return result; }
	ImmutableString ImmutableString::UpperCase(void) const { if (!text) return U""; ImmutableString result = *this; StringUpper(result.text, Length()); return result; }
	ImmutableString ImmutableString::NormalizedForm(NormalizeForm form) const { if (!text) return U""; ImmutableString result; UnicodeNormalize(text, &result.text, form); return result; }
	...
	int64 ImmutableString::ToInt64(void) const
	{
		...
		auto len = Length();
		if (!len) return 0;
		if (text[0] == U'-') { start = 1; negative = true; }
		else if (text[0] == U'+') { start = 1; }
	...
	int64 ImmutableString::ToInt64(const ImmutableString & digits, bool case_sensitive) const
	{
		...
		auto len = Length();
		if (!len) return 0;
		if (text[0] == U'-') { start = 1; negative = true; } else if (text[0] == U'+') { start = 1; }
	...
	float ImmutableString::ToFloat(void) const
	{
		...
		if (!len) return 0.0f;
		if (text[0] == U'-') { start = 1; negative = true; } else if (text[0] == U'+') { start = 1; }
		int point = FindFirst(U',');
		if (point == -1) point = FindFirst(U'.');
		if (point == -1) point = len;
		if (point != len && point != max(FindLast(U','), FindLast(U'.'))) throw InvalidFormatException();
		for (int i = start; i < point; i++) {
			if (text[i] < U'0' || text[i] > U'9') throw InvalidFormatException();
			int digit = text[i] - U'0';
			value *= 10.0f;
			value += float(digit);
		}
		float exp = 1.0f;
	...
	{
		if (CompareIgnoreCase(*this, U"true") == 0 || CompareIgnoreCase(*this, U"1") == 0) return true;
		else if (CompareIgnoreCase(*this, U"false") == 0 || CompareIgnoreCase(*this, U"0") == 0 || Length() == 0) return false;
	...
	bool operator == (widechar a, const ImmutableString & b) { return b.Length() == 1 && b[0] == a; }
	bool operator == (const ImmutableString & a, widechar b) { return a.Length() == 1 && a[0] == b; }
	bool operator == (wchar_t a, const ImmutableString & b) { return b.Length() == 1 && b[0] == a; }
	bool operator == (const ImmutableString & a, wchar_t b) { return a.Length() == 1 && a[0] == b; }
	bool operator!=(const ImmutableString & a, const ImmutableString & b) { return ImmutableString::Compare(a, b) != 0; }
	bool operator!=(const widechar * a, const ImmutableString & b) { return StringCompare(a, b) != 0; }
	bool operator!=(const ImmutableString & a, const widechar * b) { return StringCompare(a, b) != 0; }
	bool operator!=(const wchar_t * a, const ImmutableString & b) { return string::Compare(a, b) != 0; }
	...
	ImmutableString operator + (const wchar_t * a, const ImmutableString & b) { return ImmutableString(a) + b; }
	ImmutableString operator + (const ImmutableString & a, const wchar_t * b) { return a + ImmutableString(b); }
