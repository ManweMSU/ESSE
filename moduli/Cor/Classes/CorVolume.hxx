...
namespace ESSE
{
	...
	public:
		...
		template<class I> class ForwardIterator
		{
			I * _element;
		public:
			ForwardIterator(I * element) noexcept : _element(element) {}
			auto & operator * (void) const noexcept { return _element->GetValue(); }
			auto & operator * (void) noexcept { return _element->GetValue(); }
			auto & operator -> (void) const noexcept { return _element->GetValue(); }
			auto & operator -> (void) noexcept { return _element->GetValue(); }
			bool operator == (const ForwardIterator & b) const noexcept { return _element == b._element; }
			bool operator != (const ForwardIterator & b) const noexcept { return _element != b._element; }
			ForwardIterator & operator ++ (void) noexcept { _element = _element->_next; return *this; }
			ForwardIterator operator ++ (int) noexcept { ForwardIterator result(_element->_next); _element = _element->_next; return result; }
		};
		template<class I> class BackwardIterator
		{
			I * _element;
		public:
			BackwardIterator(I * element) noexcept : _element(element) {}
			auto & operator * (void) const noexcept { return _element->GetValue(); }
			auto & operator * (void) noexcept { return _element->GetValue(); }
			auto & operator -> (void) const noexcept { return _element->GetValue(); }
			auto & operator -> (void) noexcept { return _element->GetValue(); }
			bool operator == (const BackwardIterator & b) const noexcept { return _element == b._element; }
			bool operator != (const BackwardIterator & b) const noexcept { return _element != b._element; }
			BackwardIterator & operator ++ (void) noexcept { _element = _element->_prev; return *this; }
			BackwardIterator operator ++ (int) noexcept { BackwardIterator result(_element->_prev); _element = _element->_prev; return result; }
		};
		template<class L, class I> class ForwardEnumerator
		{
			L * _list;
		public:
			ForwardEnumerator(L * list) noexcept : _list(list) {}
		...
		template<class L, class I> class BackwardEnumerator
		{
			L * _list;
		public:
			BackwardEnumerator(L * list) noexcept : _list(list) {}
	...
	public:
		List(void) noexcept : _first(0), _last(0) {}
		List(const List & src) : _first(0), _last(0) { auto e = src._first; while (e) { InsertLast(e->_value); e = e->_next; } }
		List(List && src) noexcept { _first = src._first; _last = src._last; src._first = src._last = 0; }
		...
		List & operator = (List && src) noexcept
		{
			if (&src == this) return *this;
			Clear();
			_first = src._first; _last = src._last;
			src._first = 0; src._last = 0;
			return *this;
		}
		virtual string GetListType(void) const { return U"sequentia"; }
		virtual string ToStringE(ErrorContext & ectx) const noexcept override
		{
			...
			auto e = _first;
			while (e) {
				result << ConvertToString(e->_value);
				if (e->_next) result << U", ";
				e = e->_next;
			}
			result << U"]";
	...
	template<class E> class IPushPopList : public List<E>
	{
	public:
		virtual void Push(const E & value) = 0;
		virtual E Pop(void) = 0;
		IPushPopList & operator << (const E & value) { Push(value); return *this; }
		IPushPopList & operator >> (E & value) { value = Pop(); return *this; }
	};
	template<class E> class Queue : public IPushPopList<E>
	{
	public:
		virtual string GetListType(void) const override { return U"queue"; }
		...
		virtual E Pop(void) override
		{
			...
			auto value = static_cast<E &&>(first->GetValue());
			this->RemoveFirst();
			return value;
		}
	};
	template<class E> class Stack : public IPushPopList<E>
	{
	public:
		virtual string GetListType(void) const override { return U"stack"; }
		virtual void Push(const E & value) override { this->InsertLast(value); }
		virtual E Pop(void) override
		{
			auto last = this->GetLast();
			if (!last) throw InvalidStateException();
			auto value = static_cast<E &&>(last->GetValue());
			this->RemoveLast();
			return value;
		}
	};
	...
	public:
		class Element
		{
			friend class ESSE::BinaryTree<E>;
		...
		public:
			...
			bool operator == (const ForwardIterator & b) const noexcept { return _element == b._element; }
			bool operator != (const ForwardIterator & b) const noexcept { return _element != b._element; }
			ForwardIterator & operator ++ (void) noexcept { _element = _element->GetNext(); return *this; }
			ForwardIterator operator ++ (int) noexcept { ForwardIterator result(_element->GetNext()); _element = _element->GetNext(); return result; }
		};
		template<class I> class BackwardIterator
		{
			I * _element;
		public:
			...
			bool operator == (const BackwardIterator & b) const noexcept { return _element == b._element; }
			bool operator != (const BackwardIterator & b) const noexcept { return _element != b._element; }
			BackwardIterator & operator ++ (void) noexcept { _element = _element->GetPrevious(); return *this; }
			BackwardIterator operator ++ (int) noexcept { BackwardIterator result(_element->GetPrevious()); _element = _element->GetPrevious(); return result; }
		};
		template<class T, class I> class ForwardEnumerator
		{
			T * _tree;
		public:
			ForwardEnumerator(T * tree) noexcept : _tree(tree) {}
		...
		template<class T, class I> class BackwardEnumerator
		{
			T * _tree;
		public:
			BackwardEnumerator(T * tree) noexcept : _tree(tree) {}
			BackwardIterator<I> begin(void) const noexcept { return BackwardIterator<I>(_tree->GetLast()); }
			BackwardIterator<I> end(void) const noexcept { return BackwardIterator<I>(0); }
		};
	private:
		Element * _root;
		void _reballance_on_create(Element * start_from) noexcept
		{
		...
		void _reballance_on_delete(Element * start_from, Element ** direction) noexcept
		{
			while (start_from) {
		...
		void _get_element_attachment(Element * element_for, Element *** ptr) noexcept
		{
	...
	public:
		BinaryTree(void) noexcept : _root(0) {}
		...
		BinaryTree(BinaryTree && src) noexcept { _root = src._root; src._root = 0; }
		...
		{
			if (&src == this) return *this;
			Clear(); _root = src._root; src._root = 0;
			return *this;
		}
		virtual string GetTreeType(void) const { return U"arbor"; }
		virtual string ToStringE(ErrorContext & ectx) const noexcept override
		{
			ESSE_TRY_INTRO
			dynamic_string_ucs4 result;
			...
			auto e = GetFirst();
			while (e) {
				result << ConvertToString(e->GetValue());
				e = e->GetNext();
				if (e) result << U", ";
			}
			result << U"]";
	...
	{
	public:
		virtual string GetTreeType(void) const override { return U"set"; }
	...
	public:
		...
		operator string (void) const { return ConvertToString(key) + U": " + ConvertToString(value); }
	};
	template<class K, class V> class Dictionary : public BinaryTree< KeyValuePair<K, V> >
	{
	public:
		virtual string GetTreeType(void) const override { return U"dictionarium"; }
	...
	template<class K, class O> class ObjectDictionary : public Dictionary< K, oref<O> >
	{
	public:
		virtual string GetTreeType(void) const override { return U"dictionarium"; }

		bool Append(const K & key, O * object) { return Dictionary< K, oref<O> >::Append(key, object); }
		bool Update(const K & key, O * object) { return Dictionary< K, oref<O> >::Update(key, object); }
	...
	{
		uintptr _capacity, _iteration;
	public:
		explicit ObjectCache(uintptr capacity) : _capacity(capacity), _iteration(0) {}
		void Push(const K & key, O * object)
		{
			...
			if (_iteration >= _capacity) {
				...
				if (this->GetRoot()) {
					...
					BinaryTree< KeyValuePair<K, oref<O> > >::Remove(least);
					_iteration--;
				}
